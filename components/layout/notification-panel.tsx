...
interface AuditLog {
  ...
  patients?: { nom: string; prenom: string; imu: string } | { nom: string; prenom: string; imu: string }[] | null;
}
  ...
  useEffect(() => {
    ...
    async function fetchNotifs() {
      ...
      const { data } = await supabase
        .from("audit_logs")
        .select("id, action, details, timestamp, patient_id, patients(nom, prenom, imu)")
        .eq("user_id", user.id)
