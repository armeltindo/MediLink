...
import { generateIMU, cn } from "@/lib/utils";
...
export default function NouveauPatientPage() {
  ...
  const [imu, setImu]                   = useState(() => generateIMU());
  const [imuCopied, setImuCopied]       = useState(false);
  const [photoFile, setPhotoFile]       = useState<File | null>(null);
  const [photoPreview, setPhotoPreview] = useState<string | null>(null);
  const [duplicates, setDuplicates]     = useState<{ id: string; imu: string; nom: string; prenom: string; date_naissance: string }[]>([]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const { data } = await supabase
        .from("patients")
        .select("id, imu, nom, prenom, date_naissance")
  ...
  function copyIMU() {
    navigator.clipboard.writeText(imu);
    setImuCopied(true);
    setTimeout(() => setImuCopied(false), 2000);
  }
  ...
  async function onSubmit(data: PatientFormData) {
    ...
    try {
      ...
      if (photoFile) {
        ...
        const path = `patients/${imu}/photo_${Date.now()}.${ext}`;
      ...
      await supabase.from("audit_logs").insert({
        ...
        details: JSON.stringify({ imu }),
        timestamp: new Date().toISOString(),
      });

      toast({ title: "Patient enregistré", description: `${data.prenom} ${data.nom} — ${imu}` });
      router.push(`/patients/${imu}`);
    } catch (err: unknown) {
      // Handle IMU unique-constraint collision (PostgreSQL code 23505)
      ...
      if (pgErr.code === "23505") {
        ...
        toast({
          ...
          description: "Un nouveau IMU a été généré automatiquement. Veuillez soumettre à nouveau.",
        });
      } else {
    ...
    <div className="flex flex-col min-h-full bg-muted/20">
      ...
      <div className="flex-1 p-4 sm:p-6 pb-28 max-w-5xl mx-auto w-full">
        ...
        <div className="mb-6 bg-medical-green-light border border-medical-green/20 rounded-xl p-4 flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={copyIMU} className="border-medical-green/40 text-medical-green hover:bg-medical-green/10">
            {imuCopied ? <Check className="h-4 w-4 mr-1.5" /> : <Copy className="h-4 w-4 mr-1.5" />}
            {imuCopied ? "Copié !" : "Copier"}
          </Button>
        </div>
                    ...
                    {!checkingDups && duplicates.map((d) => (
                      <div key={d.id} className="flex items-center justify-between bg-white rounded px-2.5 py-1.5 border border-amber-200">
                        <span className="text-xs text-amber-900">
                          ...
                          <code className="font-mono ml-1.5 text-muted-foreground">{d.imu}</code>
                        </span>
                        <a
                          href={`/patients/${d.imu}`}
                          target="_blank"
