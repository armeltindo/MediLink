...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .imu-badge { background: rgba(255,255,255,0.2); padding: 8px 16px; border-radius: 8px; text-align: right; }
    .imu-badge .code { font-family: monospace; font-size: 14pt; font-weight: bold; }
...
<body>
  <div class="header">
    ...
    <div class="imu-badge">
      ...
      <div class="code">${patient.imu}</div>
    </div>
  </div>
  ...
  <div class="footer">
    ...
    Imprimé le ${new Date().toLocaleDateString("fr-FR")} — IMU: ${patient.imu}
  </div>
</body>
</html>`;

    // Save generated document to patient's document space
    try {
      const ts = Date.now();
      const storageKey = `${patientId}/generated/${ts}_dossier-medical.html`;
      ...
      if (!storageError) {
        await supabase.from("documents").insert({
          ...
          uploaded_by: user.id,
          description: "Export dossier médical complet — généré automatiquement",
        });
      }
    ...
    return new NextResponse(html, {
      headers: {
        ...
        "Content-Disposition": `inline; filename="dossier-${patient.imu}.html"`,
      },
    });
  } catch (error: unknown) {
